...
\author{ \\
\textbf{Performance Metrics, ISA, Basic Pipelining
}}
\date{Due Date: Sep 12, 2018 11:59 pm.\\
100 points}
...
\item Late submissions \textbf{(after 11:59 pm on 09/12/2018)} will not be accepted
...
\item Question 6 is a bonus question 
\end{itemize}
}}}
\end{center}
\vspace{0.2in}
...
%consumed for transferring data with these three approaches. 
\textbf {(5 points)}
%\item Compare the amount of energy consumed for transferring data with these three approaches if we normalize the value to the serial encoding approach. Which one is the best? At least give one example of disadvantage of the method that give us the best energy number? \textbf{(10 points)}
\item Compute the energy-delay-product of these techniques normalized to the serial communication.
%sending data with these three approaches if we normalize the value to the serial encoding approach. Which one is the best? 
\textbf{(5 points)}
\item  Give three bytes data value which will lead to worst case execution time for the proposed method. 
\textbf{(5 points)}
%\item On-chip interconnects are a main contributor to the processor energy. Besides, Last Level Caches (LLCs) incur considerable delay and energy overheads through transferring data over wires. Typically, LLCs consume about 15\% of the total processor energy. Inside LLCs, interconnections are the major source of energy consumption (about 80\%).
%      Suppose that we want to send data value of the three applications over the L2 interconnects. 
% How much processor energy will be saved (or wasted) if we transfer data these three               approaches and normalize the value to the binary encoding approach. 
%
%\textbf{(7 points)}
