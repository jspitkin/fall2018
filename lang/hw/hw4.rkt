...
(module+ test
  ...
  (test (parse `{begin 1 {+ 1 2} 3})
        (beginE (list (numE 1)
                      (plusE (numE 1) (numE 2))
                      (numE 3))))
  (test (parse `{record {a 10} {b {+ 1 2}}})
        (recordE (list 'a 'b)
                 (list (numE 10) (plusE (numE 1) (numE 2)))))
  (test (parse `{record})
        (recordE empty empty))
  (test (parse `{record {a 1}})
        (recordE (list 'a)
                 (list (numE 1))))
  (test (parse `{get {record {a 1}} a})
        (getE (recordE (list 'a)
                       (list (numE 1)))
              'a))
...
;; interp ----------------------------------------
(define (interp [exp : Exp] [env : Env] [sto : Store]) : Result
  (type-case Exp exp
                   ...
                   [(boxV l)
                    (v*s v-v
                         (override-store (cell l v-v)
                                         sto-v))]
                   [else (error 'interp "not a box")])))]
    [(beginE exps) (interp-begin exps env sto)]
    [(recordE flds exps) (interp-rec flds exps env sto)]
    [(getE rec fld)
     (with [(v sto-v) (interp rec env sto)]
           (type-case Value v
             [(recordV flds exps)
              (v*s (fetch (boxV-l (find fld
                                        flds
                                        exps))
                          sto-v)
                   sto-v)]
    ...
    [(setE rec fld exp)
     (with [(rec-v rec-sto) (interp rec env sto)]
           (type-case Value rec-v
             [(recordV flds exps)
              (with [(v sto-v) (interp exp env sto)]
                    (find-replace fld v sto-v flds exps))]
             [else (error 'interp "not a record")]))]))

(define (interp-begin [l : (Listof Exp)] [env : Env] [sto : Store]) : Result
  (cond
    [(equal? (length l) 1) (interp (first l) env sto)]
    [else (with [(v-new sto-new) (interp (first l) env sto)]
                (interp-begin (rest l) env sto-new))]))

(define (interp-rec [flds : (Listof Symbol)] [exps : (Listof Exp)] [env : Env] [sto : Store]) : Result
  (interp-rec-helper flds exps empty env sto))

(define (interp-rec-helper [flds : (Listof Symbol)] [exps : (Listof Exp)] [vals : (Listof Value)] [env : Env] [sto : Store]) : Result
  (cond
    [(empty? exps) (v*s (recordV flds vals) sto)]
    [else (with [(v-new sto-new) (interp (boxE (first exps)) env sto)]
                (interp-rec-helper flds (rest exps) (append vals (list v-new)) env sto-new))]))
...
(module+ test
  ...
  (test/exn (interp (parse `{let {[a 1]}
                    ...
                    mt-store)
            "not a box")
  (test/exn (interp (parse `{get 2 a})
                    mt-env
                    mt-store)
            "not a record")
  (test/exn (interp (parse `{set! 2 a 2})
                    mt-env
                    mt-store)
            "not a record"))
...
(define (interp-expr [exp : Exp]) : S-Exp
  (with [(v sto-v) (interp exp mt-env mt-store)]
        (type-case Value v
          [(numV n) (number->s-exp n)]
          [(closV arg body env) `function]
          [(boxV l) `box]
          [(recordV flds exps) `record])))
...
;; find ----------------------------------------------------
(define (find [fld : Symbol] [flds : (Listof Symbol)] [vals : (Listof Value)]) : Value
  (cond
    [(empty? flds) (error 'interp "no such field")]
    [else (if (symbol=? fld (first flds))
              (first vals)
              (find fld (rest flds) (rest vals)))]))

(module+ test
  (test (find 'a (list 'a 'b 'c) (list (numV 1) (numV 2) (numV 3)))
        (numV 1))
  (test (find 'b (list 'a 'b 'c) (list (numV 1) (numV 2) (numV 3)))
        (numV 2))
  (test (find 'c (list 'a 'b 'c) (list (numV 1) (numV 2) (numV 3)))
        (numV 3))
  (test/exn (find 'd (list 'a 'b 'c) (list (numV 1) (numV 2) (numV 3)))
            "no such field"))

;; find-replace --------------------------------------------------
(define (find-replace [fld : Symbol] [v : Value] [sto : Store] [flds : (Listof Symbol)] [vals : (Listof Value)]) : Result
  (cond
    [(empty? flds) (error 'interp "no such field")]
    [else (if (symbol=? fld (first flds))
              (type-case Value (first vals)
                [(boxV l) (v*s v
                               (override-store (cell l v)
                                               sto))]
                [else (error 'interp "not a box")])
              (find-replace fld v sto (rest flds) (rest vals)))]))

(module+ test
  (test/exn (find-replace 'c (numV 1) mt-store (list 'a 'b) (list (numV 2) (numV 3)))
            "no such field")
  (test/exn (find-replace 'a (numV 1) mt-store (list 'a) (list (numV 2)))
            "not a box"))
...
(define (override-store [nc : Storage] [sto : Store]) : Store
  (type-case (Listof Storage) sto
    [empty (cons nc sto)]
    [(cons c rst-sto) (if (equal? (cell-location nc) (cell-location c))
                          (cons nc rst-sto)
                          (cons c (override-store nc rst-sto)))]))
...
(module+ test
  ...
  (test (fetch 2 (override-store (cell 2 (numV 10))
                                 (override-store (cell 3 (numV 9))
                                                 mt-store)))
        (numV 10))
  (test (override-store (cell 2 (numV 10))
                        (list (cell 2 (numV 9))))
        (list (cell 2 (numV 10))))
  (test (override-store (cell 2 (numV 3))
                        (list (cell 1 (numV 5))
                              (cell 2 (numV 4))))
        (list (cell 1 (numV 5))
              (cell 2 (numV 3))))
  (test (override-store (cell 3 (numV 6))
                        (list (cell 1 (numV 5))
                              (cell 2 (numV 4))))
        (list (cell 1 (numV 5))
              (cell 2 (numV 4))
