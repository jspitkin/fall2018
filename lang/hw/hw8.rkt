...
(define (compile a-fae env)
  (type-case Exp a-fae
    ...
    [(boxE exp)
     (code-malloc1 22 (compile exp env))]
    [(unboxE box-expr)
     (code-malloc1 18 (compile box-expr env))]))
...
(module+ test
  ...
  (reset!)
  (ntest (interpx (compile
                   (parse `{{unbox {unbox {box {box {lambda {x} {+ x 12}}}}}}
                            {unbox {box {+ 1 17}}}})
                   mt-env)
                  empty-env
                  (init-k))
