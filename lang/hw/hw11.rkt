...
(define (interp [a : Exp] [env : Env]) : Value
  (type-case Exp a
    ...
    [(restE a) (type-case Value (interp a env)
                 ...
                 [else (error 'interp "not a list")])]
    [(if0E tst thn els)
     (if (equal? (interp tst env) (numV 0))
         (interp thn env)
         (interp els env))]))
...
(define (typecheck-if tst thn els tenv)
  (begin
    (unify! (typecheck tst tenv)
           (numT)
           tst)
    (unify! (typecheck thn tenv)
            (typecheck els tenv)
            thn)
    (typecheck thn tenv)))
...
(define (run-prog [exp : S-Exp]) : S-Exp
  (begin
    (typecheck (parse exp) mt-env)
    (type-case Value (interp (parse exp) mt-env)
